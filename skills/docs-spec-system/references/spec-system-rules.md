...
- `runner: autoresearch` MUST include either `autoresearch.agent` (`codex|claude`) or an explicit `runner_command` override
...
- `autoresearch.agent` launches a synthesized outer-agent command in the prepared worktree
- `runner_command` MAY override the synthesized launcher and MAY include `{autoresearch_*}` placeholders that resolve to the prepared repo/worktree/program/results paths
